                        ...
                        const JSValueRef arguments[], JSValueRef* exception)
{
    for (int i=0; i<argumentCount; ++i) {
        ...
        delete[] buf;
        if (JSValueGetType(ctx, arguments[i]) == kJSTypeObject) {
            JSValueRef prototype = JSObjectGetPrototype(ctx, JSValueToObject(ctx, arguments[i], 0));
            JSStringRef json = JSValueCreateJSONString(ctx, prototype, 2, 0);
            const int max = JSStringGetMaximumUTF8CStringSize(json);
            char *buf = new char[max];
            JSStringGetUTF8CString(json, buf, max);
            printf("%s%c", buf, i + 1 == argumentCount ? '\n' : ' ');
